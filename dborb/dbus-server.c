 ...
 */
ni_dbus_server_t *
__ni_dbus_server_open(const char *bus_type, ni_bool_t private, const char *bus_name)
{
	ni_dbus_server_t *server;

	ni_debug_dbus("%s(%s%s)", __FUNCTION__, bus_name, private? ", private" : "");

	server = xcalloc(1, sizeof(*server));
	server->connection = __ni_dbus_connection_open(bus_type, private, bus_name);
...
ni_dbus_object_t *
__ni_dbus_server_init_root(ni_dbus_server_t *server, const char *root_path, void *root_handle)
{
	ni_dbus_object_t *root;

	root = ni_dbus_object_new(&dbus_root_object_class, root_path, root_handle);
...
ni_dbus_server_open(const char *bus_type, const char *bus_name, void *root_object_handle)
{
	...
	if (!(server = __ni_dbus_server_open(bus_type, bus_name == NULL, bus_name)))
		return NULL;

	/* Translate bus name foo.bar.baz into object path /foo/bar/baz */
	__ni_dbus_server_init_root(server, __ni_dbus_server_root_path(bus_name), root_object_handle);

	return server;
}

const char *
ni_dbus_server_request_name_prefix(ni_dbus_server_t *server, const char *bus_name_prefix)
{
	return ni_dbus_connection_request_name_index(server->connection, bus_name_prefix);
}

/*
 * Create a client that shares the connection with the server object
 */
ni_dbus_client_t *
ni_dbus_server_create_shared_client(ni_dbus_server_t *server, const char *service_bus_name)
{
	ni_dbus_client_t *client;

	client = ni_dbus_client_create(server->connection, service_bus_name);
	if (client && server->root_object) {
		ni_dbus_object_t *object;

		object = ni_dbus_client_object_new(client,
				server->root_object->class,
				server->root_object->path,
				NULL, NULL);
...
const char *
ni_dbus_server_get_local_bus_name(const ni_dbus_server_t *server)
{
	return ni_dbus_connection_get_local_bus_name(server->connection);
}
 ...
 */
void
ni_dbus_server_add_signal_handler(ni_dbus_server_t *server,
					const char *sender,
					...
					void *user_data)
{
	ni_dbus_add_signal_handler(server->connection,
