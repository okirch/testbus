...
__ni_dbus_object_free(ni_dbus_object_t *object)
{
	...
	__ni_dbus_object_drop_cached_propeties(object);
...
void *
ni_dbus_object_get_handle_typecheck(const ni_dbus_object_t *object, const ni_dbus_class_t *class, DBusError *error)
{
	if (object == NULL) {
		if (error)
			dbus_set_error(error, DBUS_ERROR_FAILED,
				"cannot extract object handle for NULL object");
		return NULL;
	}
	...
	if (class == NULL) {
		if (error)
			dbus_set_error(error, DBUS_ERROR_FAILED,
				"cannot extract object handle for NULL class");
		return NULL;
	}

	if (!ni_dbus_object_isa(object, class)) {
		if (error)
			dbus_set_error(error, DBUS_ERROR_FAILED,
					"incompatible object %s of class %s (expected a %s object)",
					object->path, object->class->name, class->name);
		return NULL;
	}
	...
	if (object->handle == NULL) {
		if (error)
			dbus_set_error(error, DBUS_ERROR_FAILED,
				"NULL handle when unwrapping object %s of class %s",
 ...
 */
static const ni_dbus_property_t *
__ni_dbus_object_get_property_def(const ni_dbus_object_t *object, const char *property_path, const ni_dbus_service_t **service_p)
{
...
ni_dbus_object_get_property(const ni_dbus_object_t *object, const char *property_path, const ni_dbus_service_t *service, ni_dbus_variant_t *var)
{
	...
	property = __ni_dbus_object_get_property_def(object, property_path, &service);
	if (property == NULL)
		return FALSE;
...
/*
 * Handle cached properties
 */
ni_dbus_objprops_t *
__ni_dbus_objprops_new(const ni_dbus_service_t *service)
{
	ni_dbus_objprops_t *objprops;

	objprops = ni_malloc(sizeof(*objprops));
	objprops->service = service;
	ni_dbus_variant_init_dict(&objprops->dict);
	return objprops;
}

void
__ni_dbus_objprops_free(ni_dbus_objprops_t *objprops)
{
	ni_dbus_variant_destroy(&objprops->dict);
	free(objprops);
}

void
__ni_dbus_object_drop_cached_propeties(ni_dbus_object_t *object)
{
	while (object->cached_properties != NULL) {
		ni_dbus_objprops_t *objprops = object->cached_properties;

		object->cached_properties = objprops->next;
		__ni_dbus_objprops_free(objprops);
	}
}

const ni_dbus_variant_t *
ni_dbus_object_get_cached_property(const ni_dbus_object_t *object, const char *property_path, const ni_dbus_service_t *service)
{
	const ni_dbus_property_t *property;
	ni_dbus_objprops_t *objprops;

	if (object->cached_properties == NULL)
		return NULL;
	...
	for (objprops = object->cached_properties; objprops; objprops = objprops->next) {
		if (objprops->service == service)
			break;
	}
	...
	return ni_dbus_dict_get(&objprops->dict, property->name);
}

ni_dbus_variant_t *
__ni_dbus_object_add_cached_property(ni_dbus_object_t *object, const char *name, const ni_dbus_service_t *service)
{
	const ni_dbus_property_t *property;
	ni_dbus_objprops_t *objprops, **pos;

	if (!(property = ni_dbus_service_lookup_property(service, name))) {
		ni_error("object %s has no property named \"%s\"", object->path, name);
		return NULL;
	}

	for (pos = &object->cached_properties; (objprops = *pos) != NULL ; pos = &objprops->next) {
		if (objprops->service == service)
			break;
	}

	if (objprops == NULL)
		*pos = objprops = __ni_dbus_objprops_new(service);

	/* We use property->name instead of the name passed in, as it is persistent.
	 * Whereas the string argument @name may be from a DBusMessage, and become
	 * invalid.
	 */
	return ni_dbus_dict_add(&objprops->dict, property->name);
}
