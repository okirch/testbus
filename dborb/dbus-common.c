 ...
 */
ni_dbus_variant_t *
ni_dbus_variant_new(void)
{
	ni_dbus_variant_t *var;

	var = xcalloc(1, sizeof(*var));
	*var = (ni_dbus_variant_t) NI_DBUS_VARIANT_INIT;
	return var;
}
...
ni_dbus_variant_free(ni_dbus_variant_t *var)
{
	...
	memset(var, 0x5b, sizeof(*var));
	free(var);
}

void
ni_dbus_variant_vector_init(ni_dbus_variant_t *vars, unsigned int count)
{
	memset(vars, 0, sizeof(vars[0]) * count);
}

void
ni_dbus_variant_vector_destroy(ni_dbus_variant_t *vars, unsigned int count)
{
	while (count--)
		ni_dbus_variant_destroy(vars++);
}
...
dbus_bool_t
ni_dbus_variant_get_string_array(const ni_dbus_variant_t *var, ni_string_array_t *array)
{
	unsigned int i;
	...
	/* ni_string_array_reserve(var->array.len); */
	for (i = 0; i < var->array.len; ++i)
		ni_string_array_append(array, var->string_array_value[i]);
	return TRUE;
}
 ...
 */
ni_dbus_variant_t *
ni_dbus_dict_new(void)
{
	ni_dbus_variant_t *dict = ni_dbus_variant_new();
