...
static ni_bool_t	ni_dbus_xml_register_properties(const ni_dbus_service_t *service, ni_xs_type_t *type);
...
ni_dbus_xml_register_services(ni_xs_scope_t *scope)
{
	...
	for (xs_service = scope->services; xs_service; xs_service = xs_service->next) {
		...
		ni_dbus_xml_register_properties(service, ni_dbus_xml_get_properties_schema(scope, xs_service));
	}
...
ni_dbus_xml_register_classes(ni_xs_scope_t *scope)
{
	...
	for (xs_class = scope->classes; xs_class; xs_class = xs_class->next) {
		const ni_dbus_class_t *base_class = NULL;
		ni_dbus_class_t *new_class;

		if (xs_class->base_name) {
...
ni_dbus_property_t *ni_objectmodel_service_register_property(ni_dbus_service_t *service, const char *name, const char *signature);

ni_bool_t
ni_dbus_xml_register_properties(const ni_dbus_service_t *service, ni_xs_type_t *type)
{
	...
	if (type == NULL)
		return TRUE;
	if (type->class != NI_XS_TYPE_DICT) {
		ni_error("%s: properties must be a dict", service->name);
		return FALSE;
	}
	...
	for (i = 0; i < dict_info->children.count; ++i) {
		ni_xs_name_type_t *nt = &dict_info->children.data[i];
		const char *signature;

		if (!(signature = ni_xs_type_to_dbus_signature(nt->type))) {
			ni_error("%s: cannot build signature for property %s",
...
/*
 * Builtin notations for XML representations
 */
static inline void *
...
static ni_xs_notation_t	__ni_builtin_xs_notations[] = {
	{
		.name = "external-file",
...
ni_dbus_define_xml_notations(void)
{
	...
	for (na = __ni_builtin_xs_notations; na->name; ++na)
