...
{
	return __ni_dbus_connection_open(bus_type_string, bus_name == NULL, bus_name);
}

ni_dbus_connection_t *
__ni_dbus_connection_open(const char *bus_type_string, ni_bool_t private, const char *bus_name)
{
	ni_dbus_connection_t *connection;
	DBusError error;
	DBusBusType bus_type;

	NI_TRACE_ENTER_ARGS("bus=%s, name=%s%s", bus_type_string?: "default", bus_name?: "", private? ", private" : "");
	...
	if (private) {
		...
		if (bus_name && !ni_dbus_connection_request_name(connection, bus_name))
			goto failed;
	}
...
ni_dbus_connection_t *
ni_dbus_connection_ref(ni_dbus_connection_t *connection)
{
	ni_assert(connection);
	ni_assert(connection->refcount);
	connection->refcount++;
...
/*
 * Request the given bus_name
 */
int
__ni_dbus_connection_request_name(ni_dbus_connection_t *connection, const char *bus_name)
{
	...
	if (rv == DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER && connection->bus_name == NULL)
		ni_string_dup(&connection->bus_name, bus_name);

	return rv;
}

dbus_bool_t
ni_dbus_connection_request_name(ni_dbus_connection_t *connection, const char *bus_name)
{
	int rv;

	rv = __ni_dbus_connection_request_name(connection, bus_name);

	/* negative return value means the dbus call failed */
	if (rv < 0)
		return FALSE;

	if (rv != DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER) {
		ni_error("failed to acquire dbus name %s (rv=%d) - service already running?",
				bus_name, rv);
		return FALSE;
	}
...
const char *
ni_dbus_connection_request_name_index(ni_dbus_connection_t *conn, const char *bus_name_prefix)
{
	static char bus_name[1024];
	unsigned int index;

	for (index = 0; index < 256; ++index) {
		int rv;

		snprintf(bus_name, sizeof(bus_name), "%s%u", bus_name_prefix, index);
		rv = __ni_dbus_connection_request_name(conn, bus_name);
		if (rv == DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER)
			return xstrdup(bus_name);
		if (rv != DBUS_REQUEST_NAME_REPLY_EXISTS) {
			ni_error("failed to acquire dbus name %s (rv=%d)",
					bus_name, rv);
			return NULL;
		}
	}

	ni_error("failed to acquire dbus name %s.<index> - all names already taken", bus_name_prefix);
	return NULL;
}

const char *
ni_dbus_connection_get_local_bus_name(const ni_dbus_connection_t *connection)
{
	return connection->bus_name;
}
		...
		ni_dbus_message_t *call, unsigned int call_timeout, DBusError *error)
{
	...
	if (msgtype == DBUS_MESSAGE_TYPE_ERROR) {
		ni_dbus_set_error_from_message(error, reply);
